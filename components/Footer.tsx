  ...
  return <footer className="site-footer">
    <div className="site-shell footer-grid">
      ...
      <div className="footer-contact">
        <span className="footer-contact-label">{salesLabel}</span>
        <div className="footer-phone-grid">{site.salesPhones.map((line) => <a key={line.phone} href={`tel:${line.phone}`} dir="ltr">{line.display}</a>)}</div>
        <span className="footer-contact-label">{serviceLabel}</span>
        <a href={`tel:${site.customerService.phone}`} dir="ltr">{site.customerService.display}</a>
        <a className="footer-map-link" href={site.map} target="_blank" rel="noreferrer">{site.city}</a>
      </div>
    </div>
